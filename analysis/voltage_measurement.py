...
# Last review: 19.02.2019 by J.Seif

# To run the functions in here you'll have to use the following, where the file
# path, the width and the doping densities are passed as arguments.
# Voltage_Measurement(path + filename, W=W, Na=Na, Nd=Nd)
...
class Voltage_Measurement:
    ...
    def __init__(self, f, Na=1e16, Nd=1, W=0.02, binn=1, bkgnd_corr=0.95, bkgnd_loc='end', T=300, crop_start=0, crop_end=1, sinton_consts=(1,2,3)):
        ...
        """
        self.f = f # file path
        self.Na = Na # p-type doping, acceptor density
        self.Nd = Nd # n-type doping, donor density
        self.Ndop = np.abs(Na-Nd)
        self.W = W # width of the wafers
        self.T = T # temperature
        self.Ai = 1e16 # Ai start value
        self.Fs = 1e20 # Fs start value
        self.R = 0.
        self.A = sinton_consts[0] # Sinton constant A
        self.B = sinton_consts[1] # Sinton constant B
        self.C = sinton_consts[2] # Sinton constant C
        ...
        self.names = ['t', 'ref', 'Voc', 'PL'] # names for the columns in the data file
        self.load_data()
        # self.data = self.data()
        # self._read_data()

    def load_data(self):
        self.raw = np.genfromtxt(self.f, delimiter='\t', skip_header=1, names=self.names)
    ...
    def nxc_from_PL(self, filt=slice(None)):
        ...
        """
        TEMPERATURE DEPENDENCE - OK
        B_rad(T) is taken care of below.
        """
        def nxc_PL(x):
            # ORIGINAL
            return (np.sqrt(self.Ndop**2 + 4 * self.Ai * PL / self.B_rad(nxc=x)) - self.Ndop) / 2
            # return (np.sqrt(self.Ndop**2 + 4 * self.Ai * PL / self.B_rad(nxc=x, constant=False)) - self.Ndop) / 2

        # ORIGINAL
        guess_nxc = (np.sqrt(self.Ndop**2 + 4 * self.Ai * PL / self.B_rad(nxc=np.ones_like(PL), constant=True)) - self.Ndop) / 2

        # guess_nxc = (np.sqrt(self.Ndop**2 + 4 * self.Ai * PL / self.B_rad(nxc=np.ones_like(PL), constant=False)) - self.Ndop) / 2
